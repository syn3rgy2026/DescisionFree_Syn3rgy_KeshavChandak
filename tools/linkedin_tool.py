...
linkedin_tool.py
----------------
Real LinkedIn posting via browser automation.
...
_BROWSER_SCRIPT = os.path.join(os.path.dirname(__file__), "_linkedin_browser.py")
_PYTHON = sys.executable  # use the same python interpreter


@tool
def login_to_linkedin() -> str:
    """
    Opens LinkedIn in a real visible browser for you to log in.
...
@tool
def post_to_linkedin(image_path: str, caption: str) -> str:
    """
    Posts an image with caption to LinkedIn using real browser automation.
    Requires login_to_linkedin() to have been called first.
